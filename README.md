...
Latest Dallas import status: the generated workflow now has `525/525` queue items captured, including `522` accepted decisions and `3` edited decisions. The latest import summary reports `525` permits, `1062` inspections, `1073` eval tasks, `531` reviewed labels, and `1595` source records, with the next raw append rows at `permits.csv` row `528` and `inspections.csv` row `1065`.
