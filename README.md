...
Latest Dallas import status: the generated workflow now has `532/532` queue items captured, including `529` accepted decisions and `3` edited decisions. The latest import summary reports `532` permits, `1076` inspections, `1087` eval tasks, `538` reviewed labels, and `1616` source records, with the next raw append rows at `permits.csv` row `535` and `inspections.csv` row `1079`.
