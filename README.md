...
Latest Dallas import status: the generated workflow now has `529/529` queue items captured, including `526` accepted decisions and `3` edited decisions. The latest import summary reports `529` permits, `1070` inspections, `1081` eval tasks, `535` reviewed labels, and `1607` source records, with the next raw append rows at `permits.csv` row `532` and `inspections.csv` row `1073`.
