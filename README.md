...
Latest Dallas import status: the generated workflow now has `528/528` queue items captured, including `525` accepted decisions and `3` edited decisions. The latest import summary reports `528` permits, `1068` inspections, `1079` eval tasks, `534` reviewed labels, and `1604` source records, with the next raw append rows at `permits.csv` row `531` and `inspections.csv` row `1071`.
