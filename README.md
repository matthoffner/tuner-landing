...
Latest Dallas import status: the generated workflow now has `531/531` queue items captured, including `528` accepted decisions and `3` edited decisions. The latest import summary reports `531` permits, `1074` inspections, `1085` eval tasks, `537` reviewed labels, and `1613` source records, with the next raw append rows at `permits.csv` row `534` and `inspections.csv` row `1077`.
