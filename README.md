...
Latest Dallas import status: the generated workflow now has `534/534` queue items captured, including `531` accepted decisions and `3` edited decisions. The latest import summary reports `534` permits, `1080` inspections, `1091` eval tasks, `540` reviewed labels, and `1622` source records, with the next raw append rows at `permits.csv` row `537` and `inspections.csv` row `1083`.
