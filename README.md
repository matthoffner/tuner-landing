...
Latest Dallas import status: the generated workflow now has `527/527` queue items captured, including `524` accepted decisions and `3` edited decisions. The latest import summary reports `527` permits, `1066` inspections, `1077` eval tasks, `533` reviewed labels, and `1601` source records, with the next raw append rows at `permits.csv` row `530` and `inspections.csv` row `1069`.
