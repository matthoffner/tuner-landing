...
Latest Dallas import status: the generated workflow now has `522/522` queue items captured, including `519` accepted decisions and `3` edited decisions. The latest import summary reports `522` permits, `1056` inspections, `1067` eval tasks, `528` reviewed labels, and `1586` source records, with the next raw append rows at `permits.csv` row `525` and `inspections.csv` row `1059`.
