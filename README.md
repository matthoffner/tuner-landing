...
Latest Dallas import status: the generated workflow now has `533/533` queue items captured, including `530` accepted decisions and `3` edited decisions. The latest import summary reports `533` permits, `1078` inspections, `1089` eval tasks, `539` reviewed labels, and `1619` source records, with the next raw append rows at `permits.csv` row `536` and `inspections.csv` row `1081`.
