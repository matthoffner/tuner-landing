...
Latest Dallas import status: the generated workflow now has `521/521` queue items captured, including `518` accepted decisions and `3` edited decisions. The latest import summary reports `521` permits, `1054` inspections, `1065` eval tasks, `527` reviewed labels, and `1583` source records, with the next raw append rows at `permits.csv` row `524` and `inspections.csv` row `1057`.
