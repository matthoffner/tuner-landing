...
Latest Dallas import status: the generated workflow now has `524/524` queue items captured, including `521` accepted decisions and `3` edited decisions. The latest import summary reports `524` permits, `1060` inspections, `1071` eval tasks, `530` reviewed labels, and `1592` source records, with the next raw append rows at `permits.csv` row `527` and `inspections.csv` row `1063`.
