...
Latest Dallas import status: the generated workflow now has `523/523` queue items captured, including `520` accepted decisions and `3` edited decisions. The latest import summary reports `523` permits, `1058` inspections, `1069` eval tasks, `529` reviewed labels, and `1589` source records, with the next raw append rows at `permits.csv` row `526` and `inspections.csv` row `1061`.
