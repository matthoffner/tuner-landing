...
Latest Dallas import status: the generated workflow now has `530/530` queue items captured, including `527` accepted decisions and `3` edited decisions. The latest import summary reports `530` permits, `1072` inspections, `1083` eval tasks, `536` reviewed labels, and `1610` source records, with the next raw append rows at `permits.csv` row `533` and `inspections.csv` row `1075`.
