...
Latest Dallas import status: the generated workflow now has `520/520` queue items captured, including `517` accepted decisions and `3` edited decisions. The latest import summary reports `520` permits, `1052` inspections, `1063` eval tasks, `526` reviewed labels, and `1580` source records, with the next raw append rows at `permits.csv` row `523` and `inspections.csv` row `1055`.
