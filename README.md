...
The autonomous loop is now policy-gated against low-leverage synthetic fixture churn. When Dallas import readiness is already `ready` and coverage has no thin groups, the prompt directs Codex toward autonomy, visibility, Render reliability, real ingest mechanics, product clarity, and tests. The supervisor also rejects a synthetic `example.local` Dallas raw-row append when it is not paired with a product, autonomy, ingest, reliability, or test change.
