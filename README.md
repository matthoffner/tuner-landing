...
Latest Dallas import status: the generated workflow now has `526/526` queue items captured, including `523` accepted decisions and `3` edited decisions. The latest import summary reports `526` permits, `1064` inspections, `1075` eval tasks, `532` reviewed labels, and `1598` source records, with the next raw append rows at `permits.csv` row `529` and `inspections.csv` row `1067`.
