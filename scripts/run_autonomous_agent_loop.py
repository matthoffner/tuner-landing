    ...
    *,
    ...
    for line in lines:
        ...
        if line.startswith("- timestamp:"):
            finish_current_entry()
            current_entry["latest_handoff_timestamp"] = sanitized_policy_scalar(
                line.replace("- timestamp:", "", 1).strip()
            )
        elif line.startswith("- lane:"):
            current_entry["latest_handoff_lane"] = sanitized_policy_scalar(
                line.replace("- lane:", "", 1).strip()
            )
        elif line.startswith("- status:"):
            current_entry["latest_handoff_status"] = sanitized_policy_scalar(
...
def run_artifact_health_check(log_file: Path) -> dict[str, Any]:
    ...
    if not isinstance(degradation_details, list):
        degradation_details = []
    health_summary = sanitized_policy_scalar(artifact_health.get("summary"))
    exit_status = 0 if artifact_health_loaded(artifacts) else 1
    emit(
        log_file,
        "artifact health: "
        f"status={health_status} "
        f"summary={health_summary} "
    ...
    return {
        ...
        "artifact_health_summary": health_summary,
